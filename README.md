...
The built-in healthcheck runs `pidof keepalived` every 30s (5s timeout, 3 retries, 15s start period). It reports that the daemon started and is still present, which is what `depends_on: condition: service_healthy` waits for; it does not report a crash or a stuck VRRP. keepalived is PID 1, so a crash stops the container and shows as its exit status and restart count, and a VRRP child that dies is respawned by the parent with the probe still green. The image runs `keepalived --dont-fork --log-console --log-detail`, so every VRRP state transition, track-script success/failure line, and the `Unsafe permissions found ... - disabling` warning lands in `docker logs keepalived` (and any log shipper scraping it); watch there for what the `pidof` probe cannot see.
...
| `KeepalivedConfigUnusable` | keepalived could not use all or part of its VRRP configuration: a config file it could not open, read, find or accept as a regular non-executable file, or a config that parsed with nothing to run. An unusable main config exits and crash-loops at startup; a file pulled in by plain `include` is skipped and its directives never apply, at startup as well as on reload, while the rest of the config keeps serving; a file pulled in under `include_check` or an `includer`-family directive is fatal like the main config, and the `(Line N)` prefix on the line is what tells an include from the main file | critical |
| `KeepalivedVIPOperationRefused` | the kernel refused a virtual address, route or rule operation. A refused ADD, or an interface that disappeared, leaves this node advertising as MASTER with the VIP on no node; a refused RELEASE leaves the address on this node after it has left MASTER | critical |
